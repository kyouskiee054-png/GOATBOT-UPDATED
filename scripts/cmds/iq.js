  ...
  //for true or false questions          
    if (args[0].toLowerCase() === "torf") {
      try {
        ...
        global.SizuBot.onReaction = new Map();
        global.SizuBot.onReaction.set(info.messageID, junGod);

        setTimeout(() => {
          api.unsendMessage(info.messageID);
          global.SizuBot.onReaction.delete(info.messageID);
        }, 20000);
      } catch (error) {
        console.log(error);
      }
...
try {
  ...
  message.reply({ body: `${getLang('reply')}\n\n${text}\n\n${choices}` }, (err, info) => {
    ...
    setTimeout(() => {
      const reply = global.SizuBot.onReply.get(info.messageID);
      if (!reply.answered) {
        message.unsend(info.messageID);
        global.SizuBot.onReply.delete(info.messageID);
      }
